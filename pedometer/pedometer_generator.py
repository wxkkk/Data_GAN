...
from pedometer import constants


def build_generator():
    model = Sequential()
    model.add(Dense(constants.GENERATED_NUM * (constants.WINDOW_LENGTH // 16) * 3 * 256, input_shape=(100,)))
    model.add(BatchNormalization())
    model.add(LeakyReLU())

    model.add(Reshape((constants.GENERATED_NUM * (constants.WINDOW_LENGTH // 16), 3, 256)))

    model.add(Conv2DTranspose(256, (3, 1), strides=(2, 1), padding='same'))
    model.add(BatchNormalization())
