...
def process_to_slice_window():
    path = '../data/pedometer/20201027155210012.csv'
    ...
    for i in range(int(len(data) // constants.WINDOW_LENGTH // (1 - constants.WINDOW_REPETITIVE_RATE))):
        ...
        # print(i)
        data_wid.append(data.loc[i: i + constants.WINDOW_LENGTH - 1, [0, 1, 2]])
        label_wid.append(max(data.loc[i: i + constants.WINDOW_LENGTH - 1, 3]))
        count += 1
